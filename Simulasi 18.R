...
for(i in 1:iter){
  ...
  #Outlier: Jumlah: 2 Tipe Outlier: TC dan AO, Berurutan, T tertentu ----
  tipe.Out    = outliers(c("TC","AO"), c(50+100,50+101)) #50 adl data yg harus dihilangkan
  Outlier     = data.frame(outliers.effects(tipe.Out, n=length(Z)))  
  
  Y           = Z+2*mean(Z)*(Outlier[,1])+4*mean(Z)*(Outlier[,2]) #plot(Y, type='l') #tso(as.ts(Y))
  Yt          = as.vector(Y)
  Yt1         = lag(Yt, 1)
  Yt2         = lag(Yt, 2)
