...
for(i in 1:iter){
  ...
  #Outlier: Jumlah: 4 Tipe Outlier: AO semua, Tidak Urut, T tertentu ----
  tipe.Out    = outliers(c("AO","AO","AO","AO"), c(50+20,50+40,50+60,50+80)) #50 adl data awal yg harus dihilangkan
  Outlier     = data.frame(outliers.effects(tipe.Out, n=length(Z))) 
  
  Y           = Z+3*mean(Z)*(Outlier[,1]+Outlier[,2]+Outlier[,3]+Outlier[,4]) #plot(Y, type='l') #tso(as.ts(Y))
  Yt          = as.vector(Y)
  Yt1         = lag(Yt, 1)
  Yt2         = lag(Yt, 2)
