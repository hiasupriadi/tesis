...
for(i in 1:iter){
  ...
  #Outlier: Jumlah: 2 Tipe Outlier: AO dan AO, Tidak Urut, T tertentu ----
  tipe.Out    = outliers(c("AO","AO"), c(50+17,50+33)) #50 adl data awal yg harus dihilangkan
  Outlier     = data.frame(outliers.effects(tipe.Out, n=length(Z))) 
  
  Y           = Z+3*mean(Z)*(Outlier[,1]+Outlier[,2]) #plot(Y, type='l') #tso(as.ts(Y))
  Yt          = as.vector(Y)
  Yt1         = lag(Yt, 1)
  Yt2         = lag(Yt, 2)
